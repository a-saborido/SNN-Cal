...
and every downstream plot/diagnostic reuses the cached tensors. The network
is never re-propagated per figure.

Energy is always handled in log10 scale (matching generate_dataset.py).
A linear-energy view is derived afterwards by undoing the log10, never by
training/evaluating on linear energy.
"""
import argparse
import torch
...
def collect_test_outputs(net, predictor, te_loader, pop_size):
    """
    Propagate the whole test set through the network EXACTLY ONCE and return
